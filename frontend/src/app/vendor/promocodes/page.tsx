        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => { setError(null); setIsCreateOpen(true); }} className="inline-flex items-center gap-2 rounded-xl bg-indigo-500 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition hover:bg-indigo-400"><Plus className="h-4 w-4" /> Создать</button>
                </div>
            </div>
            ...
            <div className="mb-6 grid grid-cols-2 gap-3 xl:grid-cols-4">
                {summaryCards.map((card) => <article key={card.label} className="rounded-3xl border border-white/[0.07] bg-white/[0.03] p-5"><card.icon className="mb-4 h-5 w-5 text-indigo-300" /><p className="text-xl font-black text-white">{loading ? '—' : card.value}</p><p className="mt-1 text-xs text-white/35">{card.label}</p></article>)}
            </div>

            <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
                {(['ALL', 'ACTIVE', 'PAUSED', 'ARCHIVED'] as const).map((status) => <button key={status} onClick={() => setStatusFilter(status)} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-semibold ${statusFilter === status ? 'border-indigo-400/35 bg-indigo-500/15 text-indigo-200' : 'border-white/10 bg-white/[0.03] text-white/45'}`}>{status === 'ALL' ? 'Все' : STATUS_META[status].label}</button>)}
            </div>
            ...
            {loading && promocodes.length === 0 ? (
                ...
                <div className="flex flex-col items-center rounded-3xl border border-dashed border-white/10 px-6 py-20 text-center"><Sparkles className="mb-4 h-10 w-10 text-indigo-300/30" /><h2 className="font-bold text-white">Промокодов здесь пока нет</h2><p className="mt-2 max-w-sm text-sm text-white/35">Создайте код, задайте срок и лимит — статистика появится после первых активаций.</p></div>
            ) : (
            ...
            {isCreateOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0A0F24]/75 p-4 backdrop-blur-xl"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-white/10 bg-[#111626] p-6 shadow-2xl sm:p-8"><div className="mb-7 flex items-start justify-between"><div><h2 className="text-2xl font-bold text-white">Новый промокод</h2><p className="mt-1 text-sm text-white/40">Настройте скидку и ограничения.</p></div><button onClick={() => setIsCreateOpen(false)} className="rounded-xl bg-white/5 p-2.5 text-white/50" aria-label="Закрыть"><X className="h-5 w-5" /></button></div>
                <form onSubmit={createPromocode} className="space-y-5">
                    <label className="block text-sm text-white/50">Название<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-indigo-400/60" placeholder="Летняя скидка" /></label>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm text-white/50">Область действия<select value={form.offerId} onChange={(event) => setForm({ ...form, offerId: event.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-[#1a2031] px-4 py-3 text-white"><option value="">Все товары моего магазина</option>{offers.map((offer) => <option key={offer.id} value={offer.id}>Только: {offer.title}</option>)}</select><span className="mt-2 block text-xs leading-5 text-white/30">Код не может применяться к товарам других продавцов.</span></label><label className="text-sm text-white/50">Действует до<input type="date" value={form.validTo} onChange={(event) => setForm({ ...form, validTo: event.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white" /></label></div>
                    ...
                    <button disabled={saving} className="w-full rounded-xl bg-indigo-500 py-3.5 font-bold text-white transition hover:bg-indigo-400 disabled:opacity-50">{saving ? 'Создаём…' : 'Создать промокод'}</button>
                </form></div></div>}
        </div>
    );
}
