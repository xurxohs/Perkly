...
export default function VendorSettingsPage() {
    ...
    return <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
            ...
            <div className="space-y-5"><section className="rounded-3xl border border-white/[0.07] bg-white/[0.03] p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-indigo-300/60">Тариф</p><h2 className="mt-1 text-2xl font-black text-white">{capabilities ? TIER_NAMES[capabilities.tier] : '—'}</h2></div><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-500/10 text-indigo-300"><Sparkles className="h-5 w-5" /></div></div><div className="space-y-3">{capabilityRows.map(([label, enabled]) => <div key={String(label)} className="flex items-center justify-between text-sm"><span className="text-white/45">{label}</span>{loading ? <span className="h-4 w-8 animate-pulse rounded bg-white/5" /> : enabled ? <CheckCircle2 className="h-4 w-4 text-emerald-300" /> : <X className="h-4 w-4 text-white/20" />}</div>)}</div>{capabilities?.daysRemaining != null && <p className="mt-4 text-xs text-white/30">Осталось дней: {capabilities.daysRemaining}</p>}<Link href="/pricing" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-500/15 px-4 py-3 text-sm font-bold text-indigo-200 no-underline hover:bg-indigo-500/20">Тарифы <ExternalLink className="h-4 w-4" /></Link></section>
