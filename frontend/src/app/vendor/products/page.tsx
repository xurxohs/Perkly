...
export default function VendorProductsPage() {
    ...
    return <div className="animate-in fade-in slide-in-from-bottom-4 pb-20 duration-500">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex gap-2"><button onClick={() => void load()} disabled={loading} className="rounded-xl border border-white/10 bg-white/5 p-3 text-white/60 disabled:opacity-40" aria-label="Обновить"><RefreshCw className={`h-5 w-5 ${loading ? 'animate-spin' : ''}`} /></button><button onClick={openCreate} className="inline-flex items-center gap-2 rounded-xl bg-indigo-500 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 hover:bg-indigo-400"><Plus className="h-4 w-4" /> Добавить товар</button></div>
        </div>
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2 overflow-x-auto">{(['ALL', 'ACTIVE', 'ARCHIVED'] as const).map((item) => <button key={item} onClick={() => setStatus(item)} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-semibold ${status === item ? 'border-indigo-400/35 bg-indigo-500/15 text-indigo-200' : 'border-white/10 bg-white/[0.03] text-white/45'}`}>{item === 'ALL' ? 'Все' : item === 'ACTIVE' ? 'Активные' : 'Архив'}</button>)}</div><label className="relative"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-white/30" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Найти товар" className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-10 pr-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-indigo-400/40 sm:w-64" /></label></div>
        ...
        {cropQueue[cropIndex] && createPortal(<div className="fixed inset-0 z-[2600] flex items-center justify-center bg-[#0A0F24]/85 p-4 backdrop-blur-xl"><div className="w-full max-w-2xl rounded-[2rem] border border-white/10 bg-[#151925] p-5 shadow-2xl sm:p-7"><div className="mb-5 flex items-start justify-between"><div><h3 className="text-xl font-bold text-white">Настройте кадр</h3><p className="mt-1 text-sm text-white/40">Фото {cropIndex + 1} из {cropQueue.length} · результат 16:10</p></div><button type="button" onClick={() => { setCropQueue([]); setCropIndex(0); }} className="rounded-full bg-white/5 p-2 text-white/50"><X className="h-5 w-5" /></button></div><div className="relative aspect-[16/10] overflow-hidden rounded-3xl bg-[#0A0F24]"><img src={cropQueue[cropIndex].source} alt="Кадрирование" className="absolute inset-0 h-full w-full object-cover" style={{ transform: `translate(${cropX * 12}%, ${cropY * 12}%) scale(${cropZoom})` }} /></div><div className="mt-5 grid gap-4 sm:grid-cols-3"><label className="text-xs text-white/45">Масштаб<input type="range" min="1" max="2.2" step="0.05" value={cropZoom} onChange={(event) => setCropZoom(Number(event.target.value))} className="mt-2 w-full accent-indigo-400" /></label><label className="text-xs text-white/45">Горизонталь<input type="range" min="-1" max="1" step="0.05" value={cropX} onChange={(event) => setCropX(Number(event.target.value))} className="mt-2 w-full accent-indigo-400" /></label><label className="text-xs text-white/45">Вертикаль<input type="range" min="-1" max="1" step="0.05" value={cropY} onChange={(event) => setCropY(Number(event.target.value))} className="mt-2 w-full accent-indigo-400" /></label></div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => { setCropZoom(1); setCropX(0); setCropY(0); }} className="rounded-xl bg-white/5 px-4 py-3 text-sm font-semibold text-white/60">Сбросить</button><button type="button" disabled={uploading} onClick={() => void confirmCrop()} className="rounded-xl bg-indigo-500 px-5 py-3 text-sm font-bold text-white disabled:opacity-40">{uploading ? 'Загружаем…' : cropIndex + 1 < cropQueue.length ? 'Готово, следующее' : 'Готово'}</button></div></div></div>, document.body)}

        {isEditorOpen && createPortal(<div className="vendor-product-editor fixed inset-0 z-[2000] flex items-center justify-center bg-[#0A0F24]/75 backdrop-blur-xl sm:p-5">
            <div className="h-full w-full max-w-6xl overflow-y-auto bg-[#111626] shadow-2xl sm:max-h-[94vh] sm:rounded-[2rem] sm:border sm:border-white/10">
                ...
                <form onSubmit={saveOffer} className="grid lg:grid-cols-[minmax(0,1.25fr)_minmax(300px,.75fr)]">
                    <div className="space-y-8 p-6 sm:p-8">
                        <EditorSection number="1" title="Витрина" hint="То, что увидит покупатель в каталоге.">
                            ...
                            <Field label="Обложка 16:10">
                                <div className="mt-2 overflow-hidden rounded-3xl border border-white/10 bg-[#0A0F24]/20">
                                    <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-white/[0.06] to-transparent">
                                        ...
                                        {uploading && <div className="absolute inset-0 flex items-center justify-center bg-[#0A0F24]/55 text-sm font-semibold text-white backdrop-blur-sm"><RefreshCw className="mr-2 h-4 w-4 animate-spin" /> {uploadProgress ? `Загружаем ${uploadProgress.done + 1} из ${uploadProgress.total}` : 'Загружаем'}</div>}
                                    </div>
                        ...
                        <EditorSection number="2" title="Выдача покупки" hint="Эти данные откроются только после оплаты.">
                            <Field label="Способ выдачи"><div className="mt-2 grid gap-2 sm:grid-cols-2">{FULFILLMENT_TYPES.map((type) => <button type="button" key={type.id} onClick={() => setForm({ ...form, fulfillmentType: type.id })} className={`rounded-2xl border p-3 text-left transition-colors ${form.fulfillmentType === type.id ? 'border-indigo-400/40 bg-indigo-500/10' : 'border-white/10 bg-white/[0.025] hover:bg-white/5'}`}><div className="flex items-center justify-between"><p className="text-sm font-bold text-white">{type.name}</p>{form.fulfillmentType === type.id && <CheckCircle2 className="h-4 w-4 text-indigo-300" />}</div><p className="mt-1 text-xs text-white/35">{type.hint}</p></button>)}</div></Field>
                        ...
                        <EditorSection number="3" title="Публикация" hint="Управляйте видимостью и продвижением.">
                            <div className="grid gap-3 sm:grid-cols-3"><Toggle checked={form.isActive} onChange={(value) => setForm({ ...form, isActive: value })} label="Опубликовать" /><Toggle checked={form.isFlashDrop} onChange={(value) => setForm({ ...form, isFlashDrop: value })} label="Flash Drop" icon={<Flame className="h-4 w-4 text-orange-400" />} /><Toggle checked={form.isExclusive} onChange={(value) => setForm({ ...form, isExclusive: value })} label="Эксклюзив" icon={<Sparkles className="h-4 w-4 text-indigo-300" />} /></div>
                        </EditorSection>
                    </div>

                    <aside id="vendor-live-preview" className="scroll-mt-24 border-t border-white/[0.07] bg-[#0A0F24]/15 p-6 lg:border-l lg:border-t-0 sm:p-8">
                    ...
                    <div className="sticky bottom-0 z-20 col-span-full flex items-center justify-between gap-3 border-t border-white/[0.08] bg-[#111626]/95 px-6 py-4 backdrop-blur-xl sm:px-8">
                        ...
                        <div className="ml-auto flex gap-2"><button type="button" disabled={saving} onClick={() => setIsEditorOpen(false)} className="rounded-xl px-4 py-3 text-sm font-semibold text-white/50 hover:bg-white/5 disabled:opacity-30">Отмена</button><button disabled={!canSave} className="rounded-xl bg-indigo-500 px-5 py-3 text-sm font-bold text-white transition-colors hover:bg-indigo-400 disabled:cursor-not-allowed disabled:opacity-35">{saving ? 'Сохраняем…' : editingOffer ? 'Сохранить изменения' : 'Опубликовать товар'}</button></div>
                    </div>
                </form>
            </div>
        </div>, document.body)}
...
function EditorSection({ number, title, hint, children }: { number: string; title: string; hint: string; children: React.ReactNode }) {
    return <section><div className="mb-4 flex items-start gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-500/15 text-xs font-bold text-indigo-200">{number}</span><div><h3 className="font-bold text-white">{title}</h3><p className="mt-0.5 text-xs text-white/35">{hint}</p></div></div><div className="space-y-4">{children}</div></section>;
}
...
function Toggle({ checked, onChange, label, icon }: { checked: boolean; onChange: (value: boolean) => void; label: string; icon?: React.ReactNode }) {
    return <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className={`flex items-center gap-3 rounded-2xl border p-3 text-left ${checked ? 'border-indigo-400/30 bg-indigo-500/10' : 'border-white/10 bg-white/[0.025]'}`}><span className={`h-5 w-9 rounded-full p-0.5 transition ${checked ? 'bg-indigo-500' : 'bg-white/10'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${checked ? 'translate-x-4' : ''}`} /></span>{icon}<span className="text-sm font-semibold text-white/75">{label}</span></button>;
}
