          ...
          <section role="dialog" aria-modal="true" aria-labelledby="checkout-title" className="w-full max-w-[600px] overflow-hidden rounded-t-[30px] border border-white/[0.09] bg-[#151519] shadow-2xl sm:rounded-[28px]">
            ...
            {checkoutStep === 1 ? (
              <div className="space-y-5 p-5 sm:p-7">
                ...
                <label className="block"><span className="mb-2 block text-sm font-semibold text-white/55">Комментарий продавцу <span className="font-normal text-white/25">· необязательно</span></span><textarea value={buyerComment} onChange={(event) => setBuyerComment(event.target.value.slice(0, 1000))} rows={3} placeholder="Например, уточнение по аккаунту или активации" className="w-full resize-none rounded-2xl border border-white/[0.08] bg-white/[0.045] p-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-purple-400/35" /><span className="mt-1 block text-right text-[10px] text-white/20">{buyerComment.length}/1000</span></label>
                ...
                <button onClick={() => { hapticImpact('light'); setCheckoutStep(2); }} className="w-full rounded-2xl border-0 bg-white py-4 font-black text-black">Далее</button>
              </div>
            ) : (
              <div className="space-y-5 p-5 sm:p-7">
                ...
                <div className="flex items-center gap-2 text-xs text-white/35"><PerklyGlyph name="shield" className="h-4 w-4 text-emerald-400" /> Средства защищены до получения товара</div>
