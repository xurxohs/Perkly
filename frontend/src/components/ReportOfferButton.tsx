...
export function ReportOfferButton({ offerId }: { offerId: string }) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#0A0F24]/70 p-3 backdrop-blur-sm sm:items-center" onMouseDown={(event) => { if (event.currentTarget === event.target) setOpen(false); }}>
      <div className="w-full max-w-md rounded-[28px] bg-[#1c1c1e] p-5 shadow-2xl">
        ...
        <div className="grid grid-cols-2 gap-2">{reasons.map(([value, label]) => <button key={value} type="button" onClick={() => setCategory(value)} className={`rounded-2xl border px-3 py-3 text-left text-sm ${category === value ? 'border-indigo-400/50 bg-indigo-500/15 text-white' : 'border-white/10 bg-white/[.03] text-white/55'}`}>{label}</button>)}</div>
        <textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={2000} rows={4} placeholder="Что именно произошло? Не указывайте пароль или данные карты." className="mt-3 w-full resize-none rounded-2xl border border-white/10 bg-[#0A0F24]/25 p-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-indigo-400/50" />
