...
describe('Health API (e2e)', () => {
  ...
  beforeAll(async () => {
    const module = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: RateLimitService, useValue: { status: jest.fn().mockResolvedValue({ configured: true, ready: true }) } },
      ],
    }).compile();
  ...
  it('reports liveness without dependency details', async () => {
    ...
    expect(response.json()).toMatchObject({ status: 'ok', service: 'perkly-backend' });
  });

  it('reports readiness when database and Redis are available', async () => {
    const response = await app.inject({ method: 'GET', url: '/health/ready' });
    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ status: 'ready', checks: { database: true, redis: { ready: true } } });
  });
});
